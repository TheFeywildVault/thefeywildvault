...
// Save to backend
  const res = await fetch("https://api.thefeywildvault.com/api/inventory/add", {
    method: "POST",
  ...
  if (itemId) {
    try {
      const delRes = await fetch("https://api.thefeywildvault.com/api/courier/delete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  let storedItems = [];
  try {
    const res = await fetch("https://api.thefeywildvault.com/api/inventory", { credentials: "include" });
    if (res.ok) {
        ...
        if (match) {
          ...
          subtractBtn.onclick = async () => {
            if (count > 0) {
              ...
              if (count === 0) {
                itemDiv.remove();
                await fetch("https://api.thefeywildvault.com/api/inventory/delete", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  credentials: "include",
