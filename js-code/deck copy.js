...
// Track cards that can’t be redrawn
...
function generatefate() {
  ...
  const drawCount = parseInt(drawCountStr);
  ...
  let eligibleDeck = deckofmanything.filter(card => {
    ...
    const notAlreadyDrawn = !(card.reshuffle === "No" && nonReshuffleDrawn.has(card.id));
    return isInDeck && notAlreadyDrawn;
  });
  ...
  for (let i = 0; i < drawsToMake; i++) {
    ...
    const card = eligibleDeck.splice(randIndex, 1)[0];
    pendingDraws.push(card);
  }
  ...
  // Show "Draw Next" button if more than 1 card
...
// Hide main draw button if multi-draw
if (pendingDraws.length > 1) {
  mainDrawButton.classList.add("hidden");
}
  ...
  try {
    await fetch('https://api.thefeywildvault.com/api/deck-history', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ deckHistory: fullDrawHistory })
    });
  } catch (err) {
...
function drawNextCard() {
  ...
  // Replace result content (not append)
  resultBox.innerHTML = `
    <p id="drawCounter" class="text-sm text-gray-600 dark:text-gray-400 mb-2">
  ...
  fullDrawHistory.push(card);
  if (card.reshuffle === "No") nonReshuffleDrawn.add(card.id);

  currentDrawIndex++;

  // Update tally after each draw
  updateTally();
  saveDeckHistory();

// update rendered history
const grouped = fullDrawHistory.reduce((acc, card) => {
  if (!card || !card.name) return acc;
  if (!acc[card.name]) acc[card.name] = { name: card.name, count: 0 };
  acc[card.name].count++;
  return acc;
}, {});
renderHistory(Object.values(grouped));

  // Hide button if we're done
  if (currentDrawIndex >= pendingDraws.length) {
    document.getElementById("drawNextButton").classList.add("hidden");
...
function renderHistory(history) {
  ...
  if (!historyDiv) return; // in case element not present

  historyDiv.innerHTML = '';

  history.forEach(item => {
    if (!item || !item.name || !item.count) return; // skip bad entries

    const entry = document.createElement('div');
    entry.textContent = `${item.name} × ${item.count}`;
...
function updateTally() {
  ...
  tallyContent.innerHTML = Object.values(grouped).map(card =>
    `<p><span class="font-semibold">${card.name}</span> × ${card.count} – ${card.effect}</p>`
  ).join("");
}

// Clears only the tally/saved history (not the visible drawn cards)
// Clears tally/history both locally and on the server
async function clearTally() {
  fullDrawHistory = [];
  document.getElementById("effectTally").classList.add("hidden");
  document.getElementById("tallyContent").innerHTML = "";
  document.getElementById("drawNextButton").classList.add("hidden");

  try {
    // Tell backend to clear history for this user
    const res = await fetch('https://api.thefeywildvault.com/api/deck-history/clear', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include'
    });
    ...
    if (!res.ok) {
      console.error('Failed to clear deck history on server');
    }
  } catch (err) {
    console.error('Error clearing deck history on server:', err);
  }
...
function resetDeck() {
  ...
  document.getElementById("deckResult").innerHTML = `
    <p id="drawCounter" class="text-sm text-gray-600 dark:text-gray-400 mb-2">
      Draws Remaining: <span id="drawsRemaining">0</span>
    </p>
    <p id="placeholder-text" class="text-gray-500 italic">- Click "Draw" to test your fate -</p>
  `;
...
async function loadDeckHistory() {
  try {
    const res = await fetch('https://api.thefeywildvault.com/api/deck-history', { credentials: 'include' });
    if (!res.ok) return;
    const data = await res.json();
   if (data.deckHistory) {
  fullDrawHistory = data.deckHistory;

  // Build grouped counts before rendering
  const grouped = fullDrawHistory.reduce((acc, card) => {
    if (!card || !card.name) return acc;
    if (!acc[card.name]) acc[card.name] = { name: card.name, count: 0 };
    acc[card.name].count++;
