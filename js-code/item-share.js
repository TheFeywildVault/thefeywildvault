...
console.log("Item HTML:", itemToShare?.html);
  ...
  try {
    const res = await fetch('https://api.thefeywildvault.com/api/courier/send', {
      method: 'POST',
...
async function loadCourierInbox() {
  try {
    const res = await fetch('https://api.thefeywildvault.com/api/courier/inbox', { credentials: 'include' });
...
const itemId = li?.dataset.itemId;
  ...
  try {
    const res = await fetch('https://api.thefeywildvault.com/api/courier/delete', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
