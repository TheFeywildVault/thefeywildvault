...
const savedTally = localStorage.getItem("returnedEffects");
if (savedTally) {
  returnedEffects = JSON.parse(savedTally);
  updateEffectTally(); // Display it on page load if exists
}
...
function updateEffectTally() {
 ...
 const preferredOrder = ["Permanent", "Long Rest", "Short Rest", "Hour", "Minute", "Trigger", "Immediate", "Unknown"];
  ...
  sortedDurations.forEach(duration => {
    ...
    // Special case: Long Rest clears both Short and Long Rest
...
const durationButtonLabels = {
  ...
  "Long Rest": "Clear Short and Long Rest", // Combo handled separately
...
function clearSingleEffect(duration, index) {
  // Find all effects of this duration
  const effectsForDuration = returnedEffects.filter(e => e.duration === duration);

  if (index < 0 || index >= effectsForDuration.length) return;

  // Find the effect object to remove
  const effectToRemove = effectsForDuration[index];

  // Remove that effect from returnedEffects by reference
  returnedEffects = returnedEffects.filter(e => e !== effectToRemove);

  // Update storage and UI
  localStorage.setItem("returnedEffects", JSON.stringify(returnedEffects));
  updateEffectTally();
}

function clearMultipleDurations(durations) {
  returnedEffects = returnedEffects.filter(e => !durations.includes(e.duration));
  localStorage.setItem("returnedEffects", JSON.stringify(returnedEffects));
  updateEffectTally();
}

function clearDuration(duration) {
...
function clearTally() {
  returnedEffects = [];
  localStorage.removeItem("returnedEffects");
...
// --- Your existing tables, lookup arrays, and helper functions remain as-is ---

// Example: VBadChaosTable, BadChaosTable, ModChaosTable, GoodChaosTable, VGoodChaosTable, etc.
// Also your getRandomDamageType(), getRandomAbilityType(), etc. remain the same

// ... (all your tables and helper functions)

// Duration weights table and getWeightedRandomDuration function remain as-is

// -----------------
// NEW: generateSingleEffect() - Generates and returns a single chaos effect object, no UI update or tally change
...
// -------------
// Updated generatechaos: calls generateSingleEffect, displays one effect, adds to tally immediately
...
// -------------
// New generatecontrolledchaos: generate two options, display both with "Choose" buttons, no tally update yet
...
// -------------
// When user chooses one of the controlled chaos options, add it to the tally and update UI
function chooseControlledChaos(index) {
...
// e.g., returnedEffects loading, updateEffectTally(), clear functions, etc.
